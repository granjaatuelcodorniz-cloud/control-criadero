...
export default function Tareas() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (collabRes.data) {
        ...
        if (collabRes.data.length > 0 && !newAssigned) setNewAssigned(collabRes.data[0].id);
      }
  ...
  if (authLoading || loading) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6 text-center font-medium text-gray-400">
      Cargando tareas...
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        {!showForm ? (
          <button onClick={() => setShowForm(true)} className="w-full py-4 bg-gray-900 text-white rounded-3xl font-bold flex items-center justify-center gap-2 shadow-lg shadow-gray-200 active:scale-[0.98] transition-all">
            <Plus className="w-5 h-5" /> Nueva Tarea
          </button>
        ) : (
          <div className="bg-white rounded-3xl border-2 border-yellow-200 p-5 space-y-4 animate-in slide-in-from-top-4 duration-200">
            <div className="flex items-center justify-between">
              <h3 className="font-black text-gray-800 uppercase text-xs tracking-widest">Configurar Tarea</h3>
              <X className="w-5 h-5 text-gray-400" onClick={() => setShowForm(false)} />
            </div>
            ...
            {newType === 'custom' && collaborators.length > 0 && (
              <div className="animate-in fade-in duration-300">
                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1 text-blue-500">Asignar colaborador</label>
